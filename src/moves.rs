...
mod tests {
    ...
    #[test]
    fn display_move() {
        let moves = Moves::init();
        let x = 6;
        let y = 2;
        let m = moves.east[utils::flatten(x, y)];
        utils::print_bitboard(m);
        println!("");

        let m = moves.north[utils::flatten(x, y)];
        utils::print_bitboard(m);
        println!("");

        let m = moves.south[utils::flatten(x, y)];
        utils::print_bitboard(m);
        println!("");

        let m = moves.west[utils::flatten(x, y)];
        utils::print_bitboard(m);
        println!("");

        let m = moves.king_moves[utils::flatten(x, y)];
        utils::print_bitboard(m);
        println!("");

        let m = moves.knight_moves[utils::flatten(x, y)];
        utils::print_bitboard(m);
        println!("");
        assert!(false);
    }
